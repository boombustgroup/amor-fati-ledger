...
- Batch dimensions, sender/target index bounds, non-negative amounts, and batch-level Long overflow safety — enforced at runtime by the shared `BatchExecutionContract`; sequential executability can now be packaged into `ValidatedBatchPlan`, but none of this batch machinery is yet formally verified
